...
| `cur_unit` | 통화 코드 (`USD`, `JPY(100)`, `EUR`, `CNY` 등) | 통화 식별 |
...
#### 프로젝트 통화(USD, JPY, EUR, CNY) 매핑

| 앱 통화 | API `cur_unit` | 고시 단위 | 비고 |
|---|---|---:|---|
| USD | `USD` | 1 | `deal_bas_r` → 매매기준율 |
| JPY | `JPY(100)` | 100 | 100엔당 고시환율, 기존 `entities/currency` unit 100과 일치 |
| EUR | `EUR` | 1 | |
| CNY | `CNY` | 1 | |
...
5. JSON 배열 응답에서 USD/JPY(100)/EUR/CNY 추출
6. 콤마 제거 후 number 변환
...
- API: `JPY(100)` + 100엔당 `deal_bas_r`
- 앱: `entities/currency`의 JPY `unit: 100`
...
```text
features/load-exchange-rates   → 사용자 행동(환율 불러오기 버튼)
widgets/calculation-panel      → 계산기 UI에 버튼·상태 연결
entities/exchange-rate         → API 응답 → 도메인 환율 모델 변환
shared/api                     → HTTP 호출
shared/config                  → URL·엔드포인트 상수 (키 제외)
shared/lib                     → 문자열 환율 파싱 유틸 (필요 시)
```
...
├── shared/
│   ├── api/
│   │   ├── koreaEximExchangeRateClient.ts   # GET 호출, raw JSON 반환
│   │   ├── types.ts                         # API 응답 타입
│   │   └── index.ts
│   └── config/
│       ├── exchangeRateApi.ts               # base path, AP01 코드
│       └── index.ts                         # export 추가
├── entities/
│   └── exchange-rate/
│       ├── model/
│       │   ├── types.ts                     # ExchangeRateQuote 등
│       │   ├── parseKoreaEximRates.ts       # JSON → USD/JPY/EUR/CNY 매핑
│       │   └── parseKoreaEximRates.test.ts
│       └── index.ts
└── features/
    └── load-exchange-rates/
        ├── ui/
        │   └── LoadExchangeRatesButton.tsx  # "환율 불러오기"
        ├── model/
        │   └── loadExchangeRates.ts         # api + entity 조합, 에러 메시지
        └── index.ts
```
...
| 파일 | 변경 내용 |
|---|---|
| `features/calculate-exchange/ui/CalculateExchangeForm.tsx` | 불러온 `baseRate`를 입력란에 반영, 실패 시 수동 입력 유지 |
| `features/calculate-remittance/ui/CalculateRemittanceForm.tsx` | 불러온 `telegraphicSellingRate`(tts) 반영 |
| `widgets/calculation-panel/ui/CalculationPanel.tsx` | 환율 불러오기 feature 배치 (선택) |

### 6.3 의존 방향

```text
features/load-exchange-rates
...
features/calculate-exchange
  → entities/rate
  → entities/currency
  → (선택) features/load-exchange-rates의 결과 콜백 또는 상위 widget state
...
```ts
// shared/api
export { fetchKoreaEximExchangeRates } from './koreaEximExchangeRateClient'
...
// features/load-exchange-rates
export { LoadExchangeRatesButton } from './ui/LoadExchangeRatesButton'
export { loadExchangeRates } from './model/loadExchangeRates'
```
...
export interface ExchangeRateQuote {
  currencyCode: 'USD' | 'JPY' | 'EUR' | 'CNY'
  baseRate: number           // deal_bas_r → 환전 매매기준율
  telegraphicSellingRate: number // tts → 해외송금 전신환매도율
  quotedAt: string           // searchdate 기준
  source: 'KOREAEXIM'
}

export type ExchangeRateQuoteMap = Record<
  'USD' | 'JPY' | 'EUR' | 'CNY',
  ExchangeRateQuote
>
```
...
Phase 2. Act
  ...
  3. Vite dev proxy 설정
  4. features/load-exchange-rates
...
- [ ] `shared/api`를 통해서만 외부 환율 API 호출
- [ ] API 키가 Git에 포함되지 않음
- [ ] USD, JPY, EUR, CNY 매매기준율·전신환매도율 불러오기 가능
- [ ] JPY 100엔당 환율이 기존 계산 로직과 일치
- [ ] API 실패 시 수동 입력·계산 정상 동작
- [ ] 기존 Vitest 계산 시나리오 28개 테스트 유지 통과
- [ ] `npm run build` 성공
