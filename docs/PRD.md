...
| USD 상당액 | 송금수수료 구간 판단용 (원화 환산 금액 ÷ USD 매매기준율) |
| 송금수수료 | USD 상당액 구간표에 따라 **자동 산정**된 값 (사용자 입력 아님) |
| 전신료 | 건당 **8,000원 고정** (사용자 입력 아님) |
| 총 송금 비용 | 원화 환산 금액 + 송금수수료 + 전신료 |
...
| 전신환 스프레드율 0 미만 또는 100 초과 | 스프레드율은 0% 이상 100% 이하로 입력해주세요. |
| 우대율 0 미만 또는 100 초과 | 우대율은 0% 이상 100% 이하로 입력해주세요. |
