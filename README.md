...
| **다크모드** | 화면 우측 상단 버튼으로 라이트/다크 테마를 전환합니다. 선택한 테마는 `localStorage`에 저장되어 새로고침 후에도 유지됩니다. |
...
USD 상당액 = 원화 환산 송금원금 ÷ USD 매매기준율
송금수수료 = USD 상당액 구간별 수수료 (아래 표 참고)
총 출금액 = 원화 환산 송금원금 + 송금수수료 + 전신료
```

> **송금수수료는 선택 통화의 외화 금액 숫자가 아니라 USD 상당액 기준**으로 산정합니다.  
> JPY·IDR은 100단위 통화이므로 원화 환산 시 `÷ 통화 단위(100)`이 반영됩니다.

**USD 매매기준율(수수료 산정용):**

| 상황 | 적용 방식 |
|------|-----------|
| 환율 API 사용 성공 | API 응답의 USD `deal_bas_r`(매매기준율) 사용 |
| 수동 입력·API 실패 | **1,550원** 고정값 적용 (과제용 단순화, 실제 창구 기준과 다를 수 있음) |

API를 사용하지 않거나 USD 매매기준율을 가져오지 못한 경우 결과 화면에 `송금수수료 산정을 위해 USD 매매기준율을 1,550원으로 적용했습니다.` 안내가 표시됩니다.
...
계산 전 아래 규칙을 검증하며, 조건을 만족하지 않으면 계산하지 않고 안내 메시지를 표시합니다.

| 입력 항목 | 허용 범위 | 오류 메시지 (예시) |
|-----------|-----------|-------------------|
| 우대율 | 0% 이상 100% 이하 | 우대율은 0% 이상 100% 이하로 입력해주세요. |
| 스프레드율 (환전·해외송금) | 0% 이상 100% 이하 | 스프레드율은 0% 이상 100% 이하로 입력해주세요. |
| 매매기준율·전신환매도율 | 0보다 큰 값 | 매매기준율은 0보다 커야 합니다. / 전신환매도율은 0보다 커야 합니다. |
| 외화 금액·송금 외화 금액 | 0보다 큰 값 | 외화 금액은 0보다 커야 합니다. / 송금 외화 금액은 0보다 커야 합니다. |
| 현찰 팔 때 적용환율 | 0보다 큰 값 | 현찰 팔 때 적용환율이 0 이하가 되어 계산할 수 없습니다. |

---

## 해외송금 수수료표

송금수수료는 **선택 통화 금액이 아니라 USD 상당액** 구간을 기준으로 합니다.  
USD 상당액 = 원화 환산 송금원금 ÷ USD 매매기준율 (과제용 예시 기준이며, 실제 은행 수수료와 다를 수 있습니다.)
...
| `features` | 사용자 액션(폼, 버튼, 검색) | `calculate-exchange`, `calculate-remittance`, `load-exchange-rate`, `add-transaction`, `toggle-theme` |
